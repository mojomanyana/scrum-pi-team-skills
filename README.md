...
`packages/contracts/src/schemas/delivery-authority.schema.json` defines the versioned `spts.delivery-authority/1.0.0` structural envelope. A contract binds one Paca project/task to a repository and remote, immutable base commit/tree, one feature branch, assurance run/scope, separated Product/Flow/Principal Developer/independent Verifier/stakeholder identities, bounded autonomy, canonical action sets, workflow state, grants, recoveries, and immutable evidence. Paca remains the Scrum control plane and system of record. Contract data assumes every agent is a local Pi process governed by pi-daddy and visibly supervised through Herdr; it neither launches a process nor contacts those systems.

> **Warning:** direct AJV success proves only JSON shape. It does **not** authorize publication, merge, recovery, or any other effect. Consumers must call `validateDeliveryAuthorityContract`; effect, transition, and recovery evaluators additionally require the trusted frozen authority digest and trusted monotonic metering digest as their third and fourth arguments. Both digests must come separately from the Paca/controller boundary, never from the untrusted contract being checked. Adapters must still enforce the returned decision immediately against their own fresh observations.

The authoritative composite validator safely snapshots hostile JavaScript inputs, applies AJV structure checks, credential-scans every structurally accepted string, requires exact immutable canonical enumerations and SHA-256 authority/evidence digests, and enforces cross-field invariants. In particular, author/orchestrator/verifier/merger identities are distinct; the Verifier has read-only authority and a different execution/workspace from the author; candidate publication requires independent `APPROVE` for the exact candidate tree; the expected remote commit/tree must equal the frozen task base; candidate, remote-base, or PR-head drift invalidates publication; and merge requires a separate stakeholder grant for that tree. Stakeholder grants and requested protected effects are part of the frozen authority digest. Release, production, destructive Git, and real-Pi execution each use a distinct effect grant and cannot inherit merge or publication authority.

The workflow records explicit worktree, verifier-candidate, assurance, security, credential-scope, CI, and merge observations. Unsafe observations require their canonical escalation; merge and completion require passing CI from the latest accepted CI-monitoring boundary; and completion additionally requires a fresh merge-success receipt bound to the exact tree, pull request, merge commit, and latest accepted merge-gate boundary. The current state and candidate tree must equal the end of one contiguous, ordered accepted-transition audit. Rejected controller receipts are retained in the same ordered audit trail but do not alter or need to match that accepted state chain. Earlier transition/effect records and verification evidence retain their own candidate trees across repair attempts; publication separately requires fresh approval evidence for the current tree. Persisted allowed effect receipts are independently rechecked against their historical role authority, metering time, elapsed-duration and capacity budgets, cancellation, escalation, phase, CI observation, publication, grant, and identity gates, so audit history cannot claim an effect that applicable authoritative safety policy would deny. Repair budget and attempt-usage provenance are derived from the immutable accepted transition trace and its authority-digest-bound workflow checkpoint. The checkpoint anchors a mid-workflow trace to its starting state, candidate tree, time, active repair budget, and baseline implementation/verification/CI attempt counters. Current attempt usage must equal that frozen baseline plus accepted transitions that start later attempts. Remaining metering state—trusted controller time, elapsed duration, concurrent agents, worktrees, and sticky cancellation—is bound into a separately trusted monotonic metering digest, leaving static delivery authority frozen while metering advances. Counter rollback, cancellation rollback, trace truncation, or a CI observation reset therefore cannot restore exhausted authority or move CI-failed mutation onto another budget. Transitions that would start an implementation or repair attempt preflight its applicable limit and return `autonomy-exhausted` without advancing state. Mutating repair effects are authorized only after that transition reaches `implementation`; the counted final attempt remains executable. `validateFrozenDeliveryAuthorityContract` compares a contract to separately supplied trusted authority and monotonic metering digests. `evaluateDeliveryTransition`, `authorizeDeliveryEffect`, and `evaluateAdministrativeRecovery` are deterministic policy evaluators. They enforce that frozen anchor, the contract's active role, exact actor/execution/workspace identity, role-specific transition/effect authority, workflow phase, exact candidate tree, and applicable attempt/capacity budget. Every accepted or rejected request supplies the exact canonical controller-observed timestamp covered by the separately trusted metering digest, and that timestamp must be newer than all retained contract evidence and decisions. It is also covered by the request digest and returned in the fixed, redacted audit record with its stable idempotency key. Exact effect and recovery repeats return the prior audit receipt in an explicitly non-executable idempotent result, and historical transition repeats retain the current workflow state. Only one decision may use a global idempotency key; evaluators deny cross-namespace reuse before execution, and persisted validation independently rejects collisions across transitions, effects, recoveries, grants, and evidence. Administrative recovery is limited to the exported allowlist in a blocked or escalated workflow after exact actor, execution, and workspace revalidation and can target only an administrative gate. It cannot bypass identity, security, scope, destructive-effect, production, or exhausted-autonomy escalation gates. Active mandatory escalations cannot resume ready work, and cancellation denies every subsequent workflow transition and effect after the current atomic operation.

The complete accepted example and Product, Flow, Principal Developer, and Verifier positive examples are under `packages/contracts/examples`. The executable denied-boundary fixture matrix under `packages/contracts/test/fixtures/invalid` covers identity and worktree drift, verifier mutation, stale approval, security/assurance and credential/scope expansion, destructive/production effects, exhausted limits, unsafe paths, remote/PR drift, missing stakeholder merge authority, and attempted recovery bypass. Evidence records bind task, repository, branch, commit/tree, role/execution/workspace, assurance run/phase, verification verdict/reviewed tree/approval, PR, CI verdict/run/check, Paca update, freshness event, correlation, idempotency, and canonical digest. Merge-gate and completion require the latest exact-candidate CI evidence at the current CI-monitoring boundary to be `PASS`; a passing observation, superseded PASS, later FAIL, or run/check identity first observed at an earlier monitoring boundary is insufficient. Collection cardinalities and hostile input depth/node/string budgets are bounded before canonical snapshotting; canonical evidence bytes must exactly match reported usage and the maximum evidence authority is 10 MiB.

This package defines authority and decisions only. It does not implement Flow orchestration or live Paca, Herdr, GitHub, ACP, Git, CI, deployment, publication, or process adapters; it performs no push, PR, merge, release, deployment, Paca mutation, credential use, or Pi launch.
