...
Administrative recovery remains a separate policy. Requests use kind-specific discriminated details and can run only while blocked or escalated against the administrative gate. Product retains explicitly authorized safe recovery such as canonical digest refetch. Publication reconciliation is Flow-only and binds the exact candidate commit/tree, PR number, prior authenticated effect sequence, original idempotency key/request digest, prior unknown execution outcome, and historical publication approval. Missing, rejected, wrong-tree, wrong-identity, or previously unauthorized effects cannot be reconciled.

After frozen-contract and request-shape validation, an exact same-namespace retry is resolved by authenticated request digest before current-role or freshness checks. It returns the prior receipt as explicitly non-executable even after a legitimate role handoff. The same key with different input is a deterministic conflict, and cross-namespace key reuse remains denied.

The validators safely snapshot hostile JavaScript inputs, bound depth/nodes/collections/string bytes, credential-scan all structurally accepted strings, emit fixed redacted diagnostics, enforce normalized safe authority paths, and verify canonical evidence byte usage. Positive examples are under `packages/contracts/examples`; executable denied-boundary fixtures and controller-state adversarial fixtures are under `packages/contracts/test/fixtures/invalid`.
