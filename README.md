...
> **Trust boundary:** direct AJV or `validateDeliveryAuthorityContract` success does not authorize an effect. The latter proves canonical internal consistency, not provenance. `validateFrozenDeliveryAuthorityContract(value, expectedAuthorityDigest, expectedMeteringDigest, expectedControllerStateDigest)` requires three separately trusted caller inputs. Transition, effect, and recovery evaluators require those values as their third, fourth, and fifth arguments. In particular, callers must never copy `controllerStateDigest` from attacker-controlled contract data and pass it back as the expected digest.

The static authority digest covers immutable task, repository, role, action, limit, checkpoint, requested-effect, and stakeholder-grant authority. The monotonic metering digest separately covers trusted controller time, elapsed duration, concurrent agents, worktrees, and sticky cancellation. The domain-separated controller-state digest covers the complete contract other than its own digest field, including both preceding digests, current candidate commit/tree and publication identity, workflow state and observations, every accepted or rejected transition/effect/recovery record, evidence and correlation/freshness state, active escalations, all attempt and evidence-byte usage, cancellation, merge/completion state, and trusted time. Changing, reordering, truncating, or substituting any covered value can produce a new internal digest, but it cannot match the unchanged digest supplied by the trusted controller.

Every persisted decision carries authenticated sequence, previous-chain digest, pre-decision controller-state digest, complete historical policy context, historical-context digest, resulting-state digest, and resulting-chain digest. The frozen checkpoint domain-separates the initial sequence/chain anchor; `workflow.decisionChain` must equal the unique contiguous global history across transition, effect, and recovery arrays. Timestamps strictly increase, equal each decision's historical metering time, and cannot exceed current trusted controller time. Historical policy replay uses the retained role/execution/workspace, candidate/repository, grants, attempts, escalation/cancellation, evidence, CI, verification, publication, and merge context rather than current values. A current active-role handoff therefore does not invalidate a genuinely historical receipt.

Publication and protected effects use an explicit fail-closed state matrix. Feature push, PR update, and publication require exact-tree approval evidence and `publication-authorized`, `pr-ci-monitoring`, or `merge-gate`. Merge additionally requires `merge-gate`, current exact-candidate passing CI, and its distinct stakeholder grant. Release/tag/artifact publication and production/deployment require completed, freshly merged authority plus their own grant. Destructive Git is limited to blocked recovery state with its grant. Real-Pi execution is limited to ready or implementation with its grant. Every branch also applies identity, target-tree, trusted-time, duration, capacity, escalation, and cancellation gates; unknown combinations are denied. Merge receipts must satisfy `merge-gate boundary <= merge observedAt <= trusted controller/request time`.

Administrative recovery remains a separate policy. Requests use kind-specific discriminated details and can run only while blocked or escalated against the administrative gate. Product retains explicitly authorized safe recovery such as canonical digest refetch. Publication reconciliation is Flow-only and binds the exact candidate commit/tree, PR identity, prior authenticated effect sequence, original idempotency key/request digest, prior unknown execution outcome, and historical publication approval. Missing, rejected, wrong-tree, wrong-commit, wrong-PR, wrong-identity, or previously unauthorized effects cannot be reconciled. Live-agent worktree cleanup is not a recovery kind because the contract does not yet carry a trusted, target-specific disappearance observation.

After frozen-contract and request-shape validation, an exact same-namespace retry is resolved by authenticated request digest before current-role or freshness checks. It returns the prior receipt as explicitly non-executable even after a legitimate role handoff. The same key with different input is a deterministic conflict, and cross-namespace key reuse remains denied. Conflict diagnostics have `authentication: null`, are intentionally non-persistable, and never advance the authenticated decision chain.

The validators safely snapshot hostile JavaScript inputs, bound depth/nodes/collections/string bytes, credential-scan all structurally accepted strings, emit fixed redacted diagnostics, enforce normalized safe authority paths, and verify canonical evidence byte usage. Positive examples are under `packages/contracts/examples`; executable denied-boundary fixtures are under `packages/contracts/test/fixtures/invalid`, and controller-state adversarial regressions are executable tests in `packages/contracts/test/delivery-authority-controller-state.test.ts`.

This package defines contracts and deterministic decisions only. It performs no Flow orchestration, Paca mutation, Git/GitHub/CI operation, publication, release, deployment, credential use, process launch, or other external effect.
...
`evaluateControllerTransitionV2` creates deterministic, non-executable proposals only. Snapshots, evaluation context, evidence, accepted history, and observations are forgeable caller data. Hashes establish identity; they are not signatures. Proposals can be stale or replayed, and JavaScript inspection creates no trust. This slice grants no production, publication, merge, release, deployment, Paca mutation, process, or agent-launch authority. The additive Slice 4 controller store authenticates canonical persisted records, durably deduplicates operation/request identities, and CAS-commits structurally validated Slice 3 proposals without executing their descriptive intents. It requires an explicit trusted bootstrap root, HMAC key, and local-filesystem deployment attestation.

Slice 4 provides crash-safe atomic store behavior for cooperative processes operating inside a trusted, privately owned local filesystem namespace. It detects and fails closed on observable corruption, identity drift, unsupported conditions, and ambiguous recovery. It does not defend against concurrent malicious namespace mutation by an actor with write authority over the trusted root or its ancestors. Same-UID or host-level namespace compromise is outside the application threat model. Runtime probes reject observable unsupported conditions but do not prove filesystem semantics. The literal ancestor-ownership rule makes ordinary mixed-owner Linux paths unsupported.

Store receipts authenticate structurally validated committed data only; they do not prove evaluator provenance, execute intents, issue `TrustedConcreteLaunchDecisionV2`, or create production authority. Later separately governed slices must validate committed receipts and implement any controlled execution boundary.
