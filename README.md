...
> **Warning:** direct AJV success proves only JSON shape. It does **not** authorize publication, merge, recovery, or any other effect. Consumers must call `validateDeliveryAuthorityContract` and then use the exported pure transition/effect/recovery decision functions. Adapters must still enforce the returned decision immediately against their own fresh observations.

The authoritative composite validator safely snapshots hostile JavaScript inputs, applies AJV structure checks, credential-scans every structurally accepted string, requires exact immutable canonical enumerations and SHA-256 authority/evidence digests, and enforces cross-field invariants. In particular, author/orchestrator/verifier/merger identities are distinct; the Verifier has read-only authority and a different execution/workspace from the author; candidate publication requires independent `APPROVE` for the exact candidate tree; candidate, remote-base, or PR-head drift invalidates publication; and merge requires a separate stakeholder grant for that tree. Release, production, destructive Git, and real-Pi execution each use a distinct effect grant and cannot inherit merge or publication authority.

`evaluateDeliveryTransition`, `authorizeDeliveryEffect`, and `evaluateAdministrativeRecovery` are deterministic policy evaluators. Every accepted or rejected request returns a fixed, redacted audit record with a stable idempotency key and canonical request digest. Exact repeats return the prior decision; key reuse for different input is denied. Administrative recovery is limited to the exported allowlist after identity revalidation and can target only an administrative gate. It cannot bypass identity, security, scope, destructive-effect, production, or exhausted-autonomy escalation gates. Active mandatory escalations require the workflow to be `blocked` or `escalated`, and cancellation stops further work after the current atomic operation.

The complete accepted example and Product, Flow, Principal Developer, and Verifier positive examples are under `packages/contracts/examples`. The executable denied-boundary fixture matrix under `packages/contracts/test/fixtures/invalid` covers identity and worktree drift, verifier mutation, stale approval, security/assurance and credential/scope expansion, destructive/production effects, exhausted limits, unsafe paths, remote/PR drift, missing stakeholder merge authority, and attempted recovery bypass. Evidence records bind task, repository, branch, commit/tree, role/execution/workspace, assurance run/phase, verification verdict/reviewed tree, PR, CI, Paca update, freshness event, correlation, idempotency, and canonical digest.
